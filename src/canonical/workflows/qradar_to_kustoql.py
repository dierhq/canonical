...
from ..services.correlation_generator import correlation_generator
...
class QRadarToKustoQLConverter:
    ...
    async def convert_rule(self, request: ConversionRequest) -> ConversionResponse:
        ...
        try:
            ...
            # Step 3: Generate AI-powered conversion
...
["T1041", "T1046", "T1021"]"""
    ...
    async def _generate_ai_conversion(self, parsed_rule: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """Generate conversion using Foundation-Sec-8B through correlation generator."""
        try:
            ...
            # Use the correlation generator for ALL conversions
            conversion_result = await correlation_generator.generate_correlation_query(
                parsed_rule, 
...
Generate the complete KustoQL query:"""
    ...
    def _create_conversion_response(
        ...
        return ConversionResponse(
            ...
            metadata={
                ...
                "correlation_approach": conversion_result.get('correlation_approach', 'unknown'),
                "correlation_patterns": conversion_result.get('correlation_patterns', {}),
    ...
    def _create_enhanced_explanation(
        ...
        correlation_approach = conversion_result.get('correlation_approach')
        if correlation_approach:
            explanation_parts.append(f"Conversion approach: {correlation_approach}")
    ...
    def _create_ai_conversion_notes(
        ...
        correlation_patterns = conversion_result.get('correlation_patterns', {})
        if correlation_patterns.get('has_correlation'):
            correlation_type = correlation_patterns.get('correlation_type', 'unknown')
            notes.append(f"🔗 CORRELATION DETECTED: {correlation_type} pattern implemented")
        
        if correlation_patterns.get('time_window', {}).get('kusto_format'):
            time_window = correlation_patterns['time_window']['kusto_format']
            notes.append(f"⏱️ TIME WINDOWING: {time_window} window applied automatically")
